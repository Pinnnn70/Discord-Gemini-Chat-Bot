...
CONVERSATION_MEMORY = []
USER_PROFILES = {}
REPLIED_MESSAGES = set()  # Track message IDs that have been replied to
...
# Main bot logic
async def selfbot():
    global BOT_USER_ID, REPLIED_MESSAGES
    print_header()
    ...
    while True:
        try:
            ...
            replies_to_bot = [
                ...
                and msg.get("id") not in REPLIED_MESSAGES  # Only consider unreplied messages
            ]
            other_messages = [
                msg for msg in messages
                if msg.get("content")
            ...
            if target_message['id'] in REPLIED_MESSAGES:
                continue

            # Mark message as replied to
            REPLIED_MESSAGES.add(target_message['id'])
